...
class NewTodayDistanceWidget(
    context: Context,
    private val startX: Int,
    private val startY: Int,
) : AbstractWidget() {

    private var latestData: TodayDistance? = null
    ...
    override fun draw(canvas: Canvas, width: Float, height: Float, centerX: Float, centerY: Float) {
        ...
        canvas.drawText(
            "%.2f KM".format(latestData?.distance ?: 0),
    ...
    override fun buildSlptViewComponent(
        ...
        val textContainer = SlptLinearLayout().apply {
            add(SlptTodaySportDistanceFView())
            add(
                SlptPictureView().apply {
                    setStringPicture(".")
                }
            )
            add(SlptTodaySportDistanceLView())
            add(
                SlptPictureView().apply {
                    setStringPicture(" KM")
                }
    ...
    override fun getDataTypes(): MutableList<DataType> = mutableListOf(DataType.DISTANCE)

    override fun onDataUpdate(type: DataType, value: Any) {
        latestData = value as? TodayDistance
    }

    companion object {
        private const val ASSET_ICON_HIGH_RES = "icons/today_distance.png"
        private const val ASSET_ICON_26WC = "26wc_icons/today_distance.png"
        private const val ASSET_ICON_SLPT = "slpt_icons/today_distance.png"
    }
}
